"""
Google Maps Directions APIを使用して、指定された出発地から目的地までの徒歩での経路情報を取得し、
所要時間と距離を表示するスクリプト。また、取得した詳細な経路情報をJSONファイルとして保存します。

このスクリプトは.envファイルからGoogle Maps PlatformのAPIキーを読み込み、
Directions APIを呼び出して経路情報を取得します。デフォルトでは東京から長野への翌日の徒歩での経路を計算します。
...
def get_directions(origin, destination, dep_time, mode="walking"):
    """
    ...
    nav_request = "&".join(["=".join([str(k), str(v)]) for k, v in nav_request_params.items()])
...
def print_route_info(directions, mode):
    """
...
def save_json(data, origin, destination, dep_time, mode):
    """
...
def main():
    ...
    print(f'UNIX時間: {int(datetime.datetime.strptime(dep_time, "%Y/%m/%d %H:%M").timestamp())}')

    directions = get_directions(origin, destination, dep_time, mode)
    print_route_info(directions, mode)
    save_json(directions, origin, destination, dep_time, mode)
